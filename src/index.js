...
const createDefaultLocation = () => ({ pathname: '/', search: '', hash: '', key: randomKey(6) })
...
const createDefaultHistory = (location) => ({
  ...
  push: function(location) { this._notifyListeners(location) },
  replace: function(location) { this._notifyListeners(location) },
  _notifyListeners: function(loc) {
    this._listeners.forEach(fn => { fn(loc) })
  },
  createHref: (loc) => {
